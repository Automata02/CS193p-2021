...
class EmojiArtDocument: ObservableObject {
	...
	private func fetchBackgroundImageDataIfNecessary() {
		...
		case .url(let url):
			...
			// Prevents from previous image loading if a new one gets dragged in.
			backgroundImageFetchCancellable?.cancel()
			
			let session = URLSession.shared
			let publisher = session.dataTaskPublisher(for: url)
				.map { (data, urlResponse) in UIImage(data: data) }
			// if the publishes publishes an error it gets replaced with a nil
				.replaceError(with: nil)
				.receive(on: DispatchQueue.main)
			
			backgroundImageFetchCancellable = publisher
			// MARK: If .assign remains or .replaceError is removed would need to handle the error
